...
class EventEmiiter {
  subscribers: any;
  constructor() {
    this.subscribers: (() => void)[] = [];
  }

  function emit() {
    for (const trigger of subscribers) trigger();
  }

  function subscribe(trigger: () => void) {
    subscribers.push(trigger);
  }

  function unSubscribe(trigger: () => void) {
    subscribers = subscribers.filter((s) => s != trigger);
  }
}
