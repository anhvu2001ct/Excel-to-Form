...
class EventEmiiter<T> {
  ...
  emit() {
    for (const trigger of this.subscribers) trigger();
  }

  subscribe<T>(trigger: (val: T) => void) {
    this.subscribers.push(trigger);
  }

  unSubscribe(trigger: Function) {
    this.subscribers = subscribers.filter((s) => s != trigger);
  }
}

const modalEvent = new EventEmiiter();
...
export { Modal, subscribe, unSubscribe };
